...
#	    --bind "ctrl-d:execute(gh repo clone {} '$DOWNLOADDIR/$(echo {} | awk -F ':' '{print $2}' ) &>/dev/null &)"  \
#		--bind "ctrl-o:execute(gh repo view {} -w &>/dev/null &)"  \
#        --bind "alt-bspace:execute(rm -f $gh_fzrepo_HISTORY &>/dev/null &)" \
...
#
#
#    [[ -z "$SearchInput" ]] && echo -e -n "\nSearch yay\n\nInput Required!\nPlease Enter Initial Search Query: " && read -r SearchInput
#    [[ -z "$SearchInput" ]] && 
#
#
#
#    yay -Ss --color always --singlelineresults --sortby votes "$SearchInput" | fzf \
#        --ansi \
#        --query "/$SearchInput" \
#        --preview="echo {}" \
...
#function yaysfzf(){
#    SearchInput="$1"
#    [[ -z "$SearchInput" ]] && echo -e -n "\nSearch yay\n\nInput Required!\nPlease Enter Initial Search Query: " && read SearchInput
#        | fzf \
#            --ansi \
#            --query "/$SearchInput" \
#            --preview="echo {}" \
...
#    SearchInput="$1"
#    [[ -z "$SearchInput" ]] && echo -e -n "\nSearch yay\n\nInput Required!\nPlease Enter Initial Search Query: " && read SearchInput
#    for yaypkg in $(yay -Ss --quiet "$SearchInput");
#    do
#        infopkgs=$(yay -Si --color always --sortby votes "$yaypkg")
#        NamePkg=$(echo "$infopkgs" | grep --regexp='Name')
#        DescriptPkg=$(echo "$infopkgs" | grep --regexp='Description')
#        echo -e "$NamePkg\t:\t$DescriptPkg$formatedPKG" | awk '{$1=$2=""; print $0}'
#    done | fzf \
#            --ansi \
#            --query "$SearchInput" \
#            --preview="echo {}" \
